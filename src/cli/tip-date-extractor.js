...
commander.requiredOption(
    ...
    '-r --regex <regex>',
    'Regular expression to search date for'
  ).option(
...
commander.on('--help', function(){
  ...
  console.log('tip-date-extractor -n test/data/MERS.txt -s "_" -f YYYY-MM-DD');
  console.log('tip-date-extractor -n test/data/MERS.txt -r [0-9]{4}-[0-9]{2}-[0-9]{2} -f YYYY-MM-DD');
}).parse(process.argv);;
...
fs.readFile(commander.newick, (err, newick_data) => {
  ...
  if(commander.splitOnChar) {
    date_parser = _.partial(split_date_parser, computed_tree, commander.splitOnChar, 'last', date_format);
  } else if (commander.regex) {
    date_parser = _.partial(regex_date_parser, computed_tree, regexp, date_format);
  } 
  ...
  const mapped = _.map(tree_with_dates.get_tips(), 
    d => [d.data.name, d.decimal_date_value]
  );

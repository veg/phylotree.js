...
export default function neighborJoining(distanceMatrixArr, n, nodeList) {
	...
	if(n<=2) {
		...
		let newRow = _.concat(0,_.filter(_.map(_.range(n), k => {
			if(k != i && k !=j) {
		...
		let m = "InternalNode0";
		let internalNodes = _.filter(nodeList, x => _.includes(x, 'InternalNode'));

		if(internalNodes.length) {
			let highestNum = _.max(_.map(internalNodes, label => _.split(label, 'InternalNode')[1]))
